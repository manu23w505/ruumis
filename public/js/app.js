...
async function cargarBenefitsPublico() {
    try {
        ...
        if (ben1Valor) {
            ...
            ben1Valor.textContent = `${valorReal}${sufijoReal}`; 
        }
        ...
        if (ben2Valor) {
            ...
            ben2Valor.textContent = `${valorReal}${sufijoReal}`;
        }
        ...
        if (ben3Valor) {
            ...
            ben3Valor.textContent = `${valorReal}${sufijoReal}`;
        }
...
async function cargarStagesPublico() {
    try {
        ...
        if (!response.ok) throw new Error("Error de red al consultar stages.");
        const datos = await response.json();

        // Inyección de textos
        const stagesTitle = document.getElementById('public-stages-title');
        if (stagesTitle) stagesTitle.textContent = datos.stages_title || '';

        // Paso 1
        const s1Title = document.getElementById('public-stage1-title');
        if (s1Title) s1Title.textContent = datos.stage1_title || '';
        const s1Text = document.getElementById('public-stage1-text');
        if (s1Text) s1Text.textContent = datos.stage1_text || '';

        // Paso 2
        const s2Title = document.getElementById('public-stage2-title');
        if (s2Title) s2Title.textContent = datos.stage2_title || '';
        const s2Text = document.getElementById('public-stage2-text');
        if (s2Text) s2Text.textContent = datos.stage2_text || '';

        // Paso 3
        const s3Title = document.getElementById('public-stage3-title');
        if (s3Title) s3Title.textContent = datos.stage3_title || '';
        const s3Text = document.getElementById('public-stage3-text');
        if (s3Text) s3Text.textContent = datos.stage3_text || '';

        // Botón
        const stagesBtn = document.getElementById('public-stages-btn');
        if (stagesBtn) {
            stagesBtn.textContent = datos.stages_btn_text || '';
            stagesBtn.setAttribute('href', datos.stages_btn_link || '#');
        }

        const imgPath = datos.stages_image || 'img/placeholder.jpg';
        const publicSource = document.getElementById('public-stages-source');
        const publicImg = document.getElementById('public-stages-img');
        
        if (publicSource) {
            publicSource.setAttribute('srcset', `/${imgPath}`);
            publicSource.setAttribute('data-srcset', `/${imgPath}`);
        }
        if (publicImg) {
            publicImg.setAttribute('src', `/${imgPath}`);
            publicImg.setAttribute('data-src', `/${imgPath}`);
        }

    } catch (err) {
        console.error("Error al inyectar datos de la sección Stages:", err);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('public-stages-title')) {
        cargarStagesPublico();
    }
});